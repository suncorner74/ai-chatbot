...
export class GeminiRagProvider {
  ...
  async embed(text: string, taskType: 'RETRIEVAL_DOCUMENT' | 'RETRIEVAL_QUERY' = 'RETRIEVAL_DOCUMENT', title?: string): Promise<number[]> {
    ...
    const response = await fetch(`${API_BASE}/${model}:embedContent?key=${encodeURIComponent(env.geminiApiKey)}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: `models/${model}`, content: { parts: [{ text }] }, config: { taskType, ...(title ? { title } : {}), outputDimensionality: env.ragEmbeddingDimensions, autoTruncate: true } }) });
    if (!response.ok) throw new Error(`GEMINI_EMBEDDING_FAILED_${response.status}`);
    const result: any = await response.json(); const values = result?.embedding?.values; if (!Array.isArray(values)) throw new Error('GEMINI_EMBEDDING_EMPTY'); return values;
  }
