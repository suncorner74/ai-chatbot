...
export class ImageService {
  async generate(operation: Operation, prompt: string, image?: InputImage) {
    ...
    const text = operation === 'generate'
      ? prompt.trim()
      : `${operation === 'enhance' ? 'Enhance this image while preserving its identity, composition and important details.' : 'Edit this image according to the user request.'}\nUser request: ${prompt.trim()}`;
