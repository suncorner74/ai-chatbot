...
export class ChatService {
  ...
  async chat(userId: string, conversationId: string | undefined, userMessage: string, ragOptions: RagOptions = {}) {
    ...
    const rag = ragOptions.enabled || ragOptions.documentId || ragOptions.knowledgeBaseId
      ? await this.ragService.retrieve(userId, userMessage, { conversationId: conversation.id, ...ragOptions })
      : null;
    const messages = await this.buildMessages(conversation.id, userMessage, 'new', rag?.evidence);
    ...
    return { message: response, conversationId: conversation.id, citations: rag?.citations || [] };
  }
  ...
  async streamChat(
    ...
    const effectiveMode = mode === 'new' && lastHistoryMessage?.role === 'user' && lastHistoryMessage.content === userMessage
      ...
      : mode;
    const rag = ragOptions.enabled || ragOptions.documentId || ragOptions.knowledgeBaseId
      ? await this.ragService.retrieve(userId, userMessage, { conversationId: conversation.id, history, ...ragOptions })
      : null;
    const messages = await this.buildMessages(conversation.id, userMessage, effectiveMode, rag?.evidence);
  ...
  private async buildMessages(
    ...
    const ragInstruction = ragEvidence
      ...
      : '';
    return [
      { role: 'system', content: CHATBOT_SYSTEM_PROMPT + ragInstruction },
      ...history.map((message) => ({ role: message.role as LLMMessage['role'], content: message.content })),
      ...(mode === 'new' ? [{ role: 'user' as const, content: userMessage }] : []),
    ];
  }
}
